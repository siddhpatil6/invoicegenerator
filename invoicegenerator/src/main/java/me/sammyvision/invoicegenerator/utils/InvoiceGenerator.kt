package me.sammyvision.invoicegenerator.utils
...
import me.sammyvision.invoicegenerator.data.ModelInvoiceFooter
import me.sammyvision.invoicegenerator.data.ModelInvoiceHeader
import me.sammyvision.invoicegenerator.data.ModelInvoiceInfo
import me.sammyvision.invoicegenerator.data.ModelInvoiceItem
import me.sammyvision.invoicegenerator.data.ModelInvoicePriceInfo
import me.sammyvision.invoicegenerator.data.ModelTableHeader
import sammyvision.invoicegenerator.BuildConfig
import sammyvision.invoicegenerator.R
...
class InvoiceGenerator(private val context: Context) {
    ...
    private var headerFontColor = Font(baseFontBold, FONT_SIZE_DEFAULT)

    init {
        appFontRegular.color = BaseColor.WHITE
        appFontRegular.size = 10f
        headerFontColor.color = BaseColor.WHITE
        headerFontColor.size = 12f
    }
    ...
    private fun initInvoiceHeader(doc: Document) {
        ...
        companyHeaderCell.border = Rectangle.NO_BORDER
        companyHeaderCell.horizontalAlignment = Element.ALIGN_RIGHT
        companyHeaderCell.paddingTop = TEXT_TOP_PADDING

        contactTable.addCell(companyHeaderCell)
        val phoneCell =
            PdfPCell(
                Paragraph(
                    headerDataSource.companyName,
                    appFontRegular
                )
            )
        ...
        headerCell.paddingTop = TEXT_TOP_PADDING
        headerCell.horizontalAlignment = Element.ALIGN_RIGHT

        address.addCell(headerCell)
    ...
    private fun initBillDetails(doc: Document) {
        ...
        val gstTitle = "GST Number"
        val txtGstNumber = PdfPCell(Phrase(gstTitle, appFontLight))

        txtGstNumber.paddingTop = 15f
        txtGstNumber.border = Rectangle.NO_BORDER
        invoiceNumAndData.addCell(txtGstNumber)


        appFontRegular.color = BaseColor.BLACK
        appFontRegular.size = FONT_SIZE_DEFAULT
        val gstCell = PdfPCell(Phrase(invoiceInfoDataSource.gstNumber, appFontRegular))
        gstCell.border = Rectangle.NO_BORDER
        invoiceNumAndData.addCell(gstCell)


        appFontRegular.color = BaseColor.LIGHT_GRAY
        appFontRegular.size = 5f
        val txtDate = PdfPCell(Phrase("Date Of Issue", appFontLight))
        txtDate.paddingTop = 15f
...
/*
    ...
    private fun initItemsTable(doc: Document) {
        ...
        for (item in invoiceTableData) {
            ...
            val disAmount = PdfPCell(Phrase("${item.thirdItem}", appFontRegular))
            disAmount.border = Rectangle.NO_BORDER
            disAmount.horizontalAlignment = Rectangle.ALIGN_RIGHT
            disAmount.paddingTop = TABLE_TOP_PADDING
    ...
    private fun initPriceDetails(doc: Document) {
        ...
        val txtSubTotalCell = PdfPCell(Phrase("CGST Amount :", appFontRegular))
        ...
        val txtTaxCell = PdfPCell(Phrase("SGST Amount :", appFontRegular))
